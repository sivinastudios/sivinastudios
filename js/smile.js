...
function pointFromEvent(e){const r=stage.getBoundingClientRect();targetX=((e.clientX-r.left)/r.width-.5)*2;targetY=((e.clientY-r.top)/r.height-.5)*2;warmth.style.left=(e.clientX-r.left)+'px';warmth.style.top=(e.clientY-r.top)+'px';stage.classList.add('is-awake');startAudio()}
stage.addEventListener('pointermove',pointFromEvent,{passive:true});
stage.addEventListener('pointerenter',()=>stage.classList.add('is-awake'));
stage.addEventListener('pointerleave',()=>{targetX=0;targetY=0;stage.classList.remove('is-awake')});

function animateCamera(){currentX+=(targetX-currentX)*.045;currentY+=(targetY-currentY)*.045;layers.forEach(layer=>{const travel=Number(layer.dataset.travel||10);const x=currentX*travel;const y=currentY*travel*.52;const scale=1+travel/1100;layer.style.transform=`translate3d(${x}px,${y}px,0) scale(${scale})`});requestAnimationFrame(animateCamera)}
if(!reduce)animateCamera();
